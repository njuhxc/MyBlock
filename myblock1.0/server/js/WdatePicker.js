...
for(var $ in obj)V.$dp[$]=obj[$];$dp=V.$dp
    }
    function E(A,$,_){
...
function U(E,_){
    ...
    if(!E.el||E.el["My97Mark"]===true||E.el.disabled||(E.el==$dp.el&&Q($dp.dd)!="none"&&$dp.dd.style.left!="-1970px")){
        E.el["My97Mark"]=false;
        return
    }
    K(E);
    if(C&&E.el.nodeType==1&&E.el["My97Mark"]===undefined)$dp.el["My97Mark"]=false
        }
        function D(){
...
function S(_,$){
    ...
    if(_)if($!=null)_.style.display=$;else return S(_,"display")
        ...
        function K(H,$){
    for(var D in _)if(D.substring(0,1)!="$")$dp[D]=_[D];for(D in H)if($dp[D]!==undefined)$dp[D]=H[D];var E=$dp.el?$dp.el.nodeName:"INPUT";
